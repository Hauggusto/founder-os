    ...
    <div className={`mx-auto w-full max-w-[1500px] space-y-6 ${isProductivity ? "productivity-workspace font-mono" : ""}`}>
      ...
      {isProductivity && (
        <section className="space-y-4 [&>div]:!rounded-2xl [&>div]:!border-amber-400/20 [&>div]:!bg-[#120f18]/80">
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                <div key={`${item.kind}-${item.id}`} className="flex min-w-0 items-center gap-2 rounded-xl border border-white/[.07] bg-black/20 px-3 py-2">
                  ...
                  <span className={`min-w-0 flex-1 truncate text-xs ${item.done ? "text-emerald-200" : "text-foreground/70"}`}>{item.label}</span>
          ...
          <NextActionsBlock />
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-2 [&>div]:!rounded-2xl [&>div]:!border-amber-400/20 [&>div]:!bg-[#120f18]/80">
            <div className="[&>div>section:first-child]:hidden [&>div>section:last-child]:xl:col-span-2"><FutureVisionBlock /></div>
            <AgendaBlock />
          </div>
        </section>
      )}
