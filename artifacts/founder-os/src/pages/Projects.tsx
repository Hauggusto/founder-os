  ...
  return (
    <div className="max-w-[1600px]">
      ...
      <section className="mb-6 rounded-2xl border border-primary/20 bg-card/50 p-4">
        ...
        {ecosystems.length === 0 ? <div className="rounded-xl border border-dashed border-white/10 p-4 text-center text-xs text-muted-foreground">Crie seu primeiro ecossistema para começar a organizar os projetos.</div> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{ecosystems.map((ecosystem) => { const linked = allProjects.filter((project) => project.ecosystem === ecosystem.id || project.category?.trim().toLowerCase() === ecosystem.name.trim().toLowerCase()); return <article key={ecosystem.id} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { const projectId = event.dataTransfer.getData("project-id"); if (projectId) updateModule(projectId, { ecosystem: ecosystem.id }); }} onClick={() => { setSelectedEcosystemId(ecosystem.id); setLocation(`/projetos?ecosystem=${encodeURIComponent(ecosystem.id)}`); }} style={{ borderColor: `${ecosystem.color}99`, boxShadow: `inset 0 1px 0 ${ecosystem.color}33` }} className="min-h-32 cursor-pointer rounded-xl border bg-background/40 p-3 transition hover:brightness-110"><div className="mb-3 flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full shadow-[0_0_8px_currentColor]" style={{ backgroundColor: ecosystem.color, color: ecosystem.color }} /><h3 className="text-xs font-semibold" style={{ color: ecosystem.color }}>{ecosystem.name}</h3><span className="ml-auto text-[10px] text-muted-foreground">{linked.length} projetos</span></div>{linked.length ? <div className="flex flex-wrap gap-2">{linked.map((project) => <Link key={project.id} href={`/projetos?project=${encodeURIComponent(project.title)}`} onClick={(event) => event.stopPropagation()} style={{ borderColor: `${ecosystem.color}66`, backgroundColor: `${ecosystem.color}12` }} className="rounded-lg border px-2.5 py-2 text-xs hover:brightness-125">{project.title}</Link>)}</div> : <p className="rounded-lg border border-dashed border-white/10 p-3 text-center text-[11px] text-muted-foreground">Solte um projeto aqui</p>}</article>; })}</div>}
