  ...
  return (
    <div className="max-w-[1600px]">
      ...
      <section className="mb-6 rounded-2xl border border-primary/20 bg-card/50 p-4">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><Layers3 className="h-5 w-5 text-primary" /><div><h2 className="text-sm font-semibold">Ecossistemas</h2><p className="text-xs text-muted-foreground">Crie uma frente e arraste os projetos correspondentes para dentro dela.</p></div></div><div className="flex items-center gap-2"><input value={newEcosystem} onChange={(event) => setNewEcosystem(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") createEcosystem(); if (event.key === "Escape") setNewEcosystem(""); }} placeholder="Nome do ecossistema" className="h-9 rounded-lg border border-border bg-background px-3 text-xs outline-none focus:border-primary" /><input aria-label="Cor do ecossistema" type="color" value={newEcosystemColor} onChange={(event) => setNewEcosystemColor(event.target.value)} className="h-9 w-10 cursor-pointer rounded-lg border border-border bg-background p-1" /><Button onClick={createEcosystem} size="sm" className="bg-primary">+ Criar</Button></div></div>
