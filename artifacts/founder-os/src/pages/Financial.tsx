...
function UpcomingFinanceCard({ kind, title, subtitle, accent, transactions, accounts, onAdd, onUpdate, onDelete }: { kind: FinancialTransaction['type']; title: string; subtitle: string; accent: string; transactions: FinancialTransaction[]; accounts: string[]; onAdd: (transaction: Omit<FinancialTransaction, 'id'>) => void; onUpdate: (id: string, updates: Partial<FinancialTransaction>) => void; onDelete: (id: string) => void }) {
  const [form, setForm] = useState({ description: '', amount: '', category: 'Não categorizado', date: new Date(Date.now() + 15 * 86400000).toISOString().slice(0, 10), account: accounts[0] || 'Conta principal' });
...
function TransactionPanel({ title, subtitle, transactions, accent, icon, categoryOptions, onUpdate, onDelete }: { title: string; subtitle: string; transactions: FinancialTransaction[]; accent: string; icon: React.ReactNode; categoryOptions: string[]; onUpdate: (id: string, updates: Partial<FinancialTransaction>) => void; onDelete: (id: string) => void }) {
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-card-border bg-card">
      ...
      <div className="divide-y divide-[#ffffff08]">
        {latest.map((transaction) => <div key={transaction.id} className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-x-3 gap-y-1 px-5 py-3 transition-colors hover:bg-white/[0.02]"><div className="min-w-0"><input value={transaction.description} onChange={(event) => onUpdate(transaction.id, { description: event.target.value })} className="w-full truncate bg-transparent text-xs font-medium text-foreground outline-none focus:border-b focus:border-primary" /><div className="mt-1 flex min-w-0 items-center gap-2"><StyledSelect value={transaction.category} onChange={(value) => onUpdate(transaction.id, { category: value })} options={categoryOptions.map((category) => [category, category] as [string, string])} compact /><span className="truncate text-[10px] text-muted-foreground" title={transaction.account}>{transaction.account}</span></div></div><div className="text-right"><label className="flex items-center justify-end gap-1"><span className="text-[10px] text-muted-foreground">R$</span><input type="text" inputMode="decimal" value={formatCurrency(transaction.amount)} onFocus={(event) => event.currentTarget.select()} onChange={(event) => onUpdate(transaction.id, { amount: parseCurrency(event.target.value) })} className="w-28 bg-transparent text-right text-xs font-semibold outline-none focus:border-b focus:border-primary" style={{ color: accent }} /></label><p className="mt-1 text-[10px] text-muted-foreground">{formatTransactionDate(transaction.date)}</p></div><button type="button" onClick={() => onDelete(transaction.id)} className="rounded-md p-1.5 text-muted-foreground transition hover:bg-red-500/10 hover:text-red-300" aria-label={`Excluir ${transaction.description}`}><Trash2 className="h-3.5 w-3.5" /></button></div>)}
