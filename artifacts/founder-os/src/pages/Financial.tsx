      ...
      <section className="mb-8 overflow-hidden rounded-lg border border-card-border bg-card">
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[760px]">
            <div className="grid grid-cols-[minmax(180px,1.6fr)_minmax(82px,0.7fr)_minmax(120px,0.9fr)_max-content_minmax(135px,0.9fr)_28px] gap-3 border-b border-[#ffffff08] px-5 py-2.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
              {([['description', 'Descrição'], ['date', 'Data'], ['category', 'Categoria'], ['account', 'Conta'], ['amount', 'Valor']] as const).map(([sort, label]) => <button key={sort} type="button" onClick={() => toggleStatementSort(sort)} className={`w-full transition-colors hover:text-primary ${sort === 'amount' ? 'text-right' : 'text-left'}`}>{label}{statementSort === sort ? <span className="ml-1 text-primary">{statementSortDirection === 'asc' ? '↑' : '↓'}</span> : null}</button>)}<span />
            </div>
...
function TransactionPanelRow({ transaction, accent, editableCategories, onUpdate, onDelete, onAddCategory }: { transaction: FinancialTransaction; accent: string; editableCategories: string[]; onUpdate: (id: string, updates: Partial<FinancialTransaction>) => void; onDelete: (id: string) => void; onAddCategory: (id: string) => void }) {
  ...
  return <div className="grid items-center gap-3 px-5 py-3 sm:grid-cols-[minmax(140px,1.3fr)_minmax(135px,1fr)_minmax(85px,0.7fr)_145px_20px]"><InlineDescriptionInput value={transaction.description} onCommit={(description) => onUpdate(transaction.id, { description })} className="min-w-0 truncate bg-transparent text-xs font-medium text-foreground outline-none focus:border-b focus:border-primary" />{editingCategory ? <StyledSelect value={transaction.category} onChange={(value) => { if (value === '__add__') onAddCategory(transaction.id); else { onUpdate(transaction.id, { category: value }); setEditingCategory(false); } }} options={categories} onAddOption={() => onAddCategory(transaction.id)} compact /> : <button type="button" onClick={() => setEditingCategory(true)} className="min-w-0 truncate text-left text-[11px] text-muted-foreground transition-colors hover:text-primary" title="Clique para alterar a categoria">{transaction.category}</button>}<span className="inline-flex min-w-0 max-w-full items-center truncate rounded-full border px-2.5 py-1 text-[11px] font-medium" style={{ color: accountTone.foreground, backgroundColor: accountTone.background, borderColor: accountTone.border }} title={`Conta de origem: ${transaction.account}`}>{transaction.account}</span><div className="text-right"><label className="flex items-center justify-end gap-1"><input type="text" inputMode="decimal" value={formatCurrency(transaction.amount)} onFocus={(event) => event.currentTarget.select()} onChange={(event) => onUpdate(transaction.id, { amount: parseCurrency(event.target.value) })} className="w-32 bg-transparent text-right text-xs font-semibold outline-none focus:border-b focus:border-primary" style={{ color: accent }} aria-label="Valor" /></label><p className="mt-1 text-[11px] text-muted-foreground">{formatTransactionDate(transaction.date)}</p></div><button type="button" onClick={() => onDelete(transaction.id)} className="rounded-md p-1.5 text-muted-foreground transition hover:bg-red-500/10 hover:text-red-300" aria-label={`Excluir ${transaction.description}`}><Trash2 className="h-3.5 w-3.5" /></button></div>;
}
...
function StatementRow({ transaction, categoryOptions, accountOptions, onUpdate, onDelete }: { transaction: FinancialTransaction; categoryOptions: string[]; accountOptions: string[]; onUpdate: (id: string, updates: Partial<FinancialTransaction>) => void; onDelete: (id: string) => void }) {
  ...
  return <div className="grid grid-cols-[minmax(180px,1.6fr)_minmax(82px,0.7fr)_minmax(120px,0.9fr)_max-content_minmax(135px,0.9fr)_28px] items-center gap-3 px-5 py-3 transition-colors hover:bg-white/[0.02]"><div className="flex min-w-0 items-center gap-2.5"><span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${isIncome ? 'bg-[#10B981]/10 text-[#10B981]' : 'bg-[#F97316]/10 text-[#F97316]'}`}>{isIncome ? <ArrowDownLeft className="h-3.5 w-3.5" /> : <ArrowUpRight className="h-3.5 w-3.5" />}</span><InlineDescriptionInput value={transaction.description} onCommit={(description) => onUpdate(transaction.id, { description })} className="min-w-0 w-full truncate bg-transparent text-xs font-medium text-foreground outline-none focus:border-b focus:border-primary" /></div><span className="text-[11px] text-muted-foreground">{formatTransactionDate(transaction.date)}</span>{editingCategory ? <StyledSelect value={transaction.category} onChange={(value) => { onUpdate(transaction.id, { category: value }); setEditingCategory(false); }} options={categories} compact /> : <button type="button" onClick={() => setEditingCategory(true)} className="min-w-0 truncate text-left text-[11px] text-muted-foreground transition-colors hover:text-primary" title="Clique para alterar a categoria">{transaction.category}</button>}{editingAccount ? <StyledSelect value={transaction.account} onChange={(value) => { onUpdate(transaction.id, { account: value }); setEditingAccount(false); }} options={accounts} compact /> : <button type="button" onClick={() => setEditingAccount(true)} className="inline-flex w-fit min-w-[76px] max-w-[180px] items-center truncate rounded-full border px-2.5 py-1 text-left text-[11px] font-medium transition hover:brightness-125" style={{ color: accountTone.foreground, backgroundColor: accountTone.background, borderColor: accountTone.border }} title="Clique para alterar a conta">{transaction.account}</button>}<label className="flex min-w-0 items-center justify-end"><input type="text" inputMode="decimal" value={formatCurrency(transaction.amount)} onFocus={(event) => event.currentTarget.select()} onChange={(event) => onUpdate(transaction.id, { amount: parseCurrency(event.target.value) })} className={`min-w-0 w-full bg-transparent text-right text-xs font-semibold outline-none focus:border-b focus:border-primary ${isIncome ? 'text-[#10B981]' : 'text-[#F97316]'}`} /></label><button type="button" onClick={() => onDelete(transaction.id)} className="text-muted-foreground hover:text-red-300" aria-label={`Excluir ${transaction.description}`}><Trash2 className="h-4 w-4" /></button></div>;
}
...
function InlineCategorySelect({ value, options, onCommit }: { value: string; options: string[]; onCommit: (value: string) => void }) {
  const [editing, setEditing] = useState(false);
  const available = Array.from(new Set([value, ...options]));
  if (editing) return <StyledSelect value={value} onChange={(next) => { onCommit(next); setEditing(false); }} options={available.map((option) => [option, option] as [string, string])} compact className="w-full" />;
  return <button type="button" onClick={() => setEditing(true)} className="min-w-0 truncate text-left text-[11px] text-muted-foreground transition-colors hover:text-primary" title="Clique para alterar a categoria">{value}</button>;
}

function InlineDescriptionInput({ value, onCommit, className = '' }: { value: string; onCommit: (value: string) => void; className?: string }) {
  const [draft, setDraft] = useState(value);
  useEffect(() => setDraft(value), [value]);
  return <input value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); onCommit(draft.trim()); event.currentTarget.blur(); } if (event.key === 'Escape') { event.preventDefault(); setDraft(value); event.currentTarget.blur(); } }} className={className} aria-label="Descrição" />;
}
