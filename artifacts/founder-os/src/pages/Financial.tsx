        ...
        <div className="overflow-x-auto">
          <div className="min-w-[760px]">
            ...
            <div className="max-h-[560px] overflow-y-auto overscroll-contain divide-y divide-[#ffffff08] [scrollbar-color:rgba(0,201,255,.45)_rgba(255,255,255,.04)] [scrollbar-width:thin]">
              {statementTransactions.map((transaction) => <StatementRow key={transaction.id} transaction={transaction} categoryOptions={Array.from(new Set([...FINANCIAL_CATEGORIES, ...INCOME_CATEGORIES, ...EXPENSE_CATEGORIES]))} onUpdate={updateTransaction} onDelete={deleteTransaction} />)}
            </div>
          </div>
        </div>
      </section>
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-card-border bg-card">
      ...
      <div className="divide-y divide-[#ffffff08]">
        {latest.map((transaction) => <div key={transaction.id} className="grid items-center gap-3 px-5 py-3 sm:grid-cols-[minmax(140px,1.3fr)_minmax(135px,1fr)_minmax(85px,0.7fr)_145px_20px]"><input value={transaction.description} onChange={(event) => onUpdate(transaction.id, { description: event.target.value })} className="min-w-0 truncate bg-transparent text-xs font-medium text-foreground outline-none focus:border-b focus:border-primary" aria-label="Descrição" /><StyledSelect value={transaction.category} onChange={(value) => value === '__add__' ? addCategory(transaction.id) : onUpdate(transaction.id, { category: value })} options={[...editableCategories, transaction.category].filter((category, index, all) => all.indexOf(category) === index).map((category) => [category, category] as [string, string])} onAddOption={() => addCategory(transaction.id)} compact /><span className="truncate text-[10px] text-muted-foreground" title={transaction.account}>{transaction.account}</span><div className="text-right"><label className="flex items-center justify-end gap-1"><span className="text-[10px] text-muted-foreground">R$</span><input type="text" inputMode="decimal" value={formatCurrency(transaction.amount)} onFocus={(event) => event.currentTarget.select()} onChange={(event) => onUpdate(transaction.id, { amount: parseCurrency(event.target.value) })} className="w-32 bg-transparent text-right text-xs font-semibold outline-none focus:border-b focus:border-primary" style={{ color: accent }} aria-label="Valor" /></label><p className="mt-1 text-[10px] text-muted-foreground">{formatTransactionDate(transaction.date)}</p></div><button type="button" onClick={() => onDelete(transaction.id)} className="rounded-md p-1.5 text-muted-foreground transition hover:bg-red-500/10 hover:text-red-300" aria-label={`Excluir ${transaction.description}`}><Trash2 className="h-3.5 w-3.5" /></button></div>)}
