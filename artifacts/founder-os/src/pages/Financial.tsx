...
function ComparisonCard({ currentWeek, previousWeek, currentMonth, previousMonth }: { currentWeek: number; previousWeek: number; currentMonth: number; previousMonth: number }) {
  ...
  return (
    <div className="rounded-lg border border-card-border bg-card p-5" style={{ boxShadow: `0 0 16px ${accent}08` }}>
      ...
      <div className="mt-3 flex items-center justify-between border-t border-[#ffffff0a] pt-3 text-[10px] text-muted-foreground"><span>Atual <strong className="ml-1 text-foreground">R$ {current.toLocaleString('pt-BR')}</strong></span><span>Anterior <strong className="ml-1 text-foreground">R$ {previous.toLocaleString('pt-BR')}</strong></span></div>
    </div>
  );
}
