    ...
    <div className="mt-3 rounded-lg border border-primary/25 bg-primary/[0.04] p-3" onClick={(event) => event.preventDefault()}>
      ...
      <div className="max-h-64 space-y-1.5 overflow-y-auto pr-1">
        {rows.map((row, index) => <div key={`${row.date}-${index}`} className="grid gap-1.5 rounded-md border border-border/60 bg-background/60 p-2 sm:grid-cols-[1.5fr_0.7fr_0.7fr_0.85fr]">
          ...
          <input value={row.amount} onChange={(event) => updateRow(index, { amount: Math.abs(Number(event.target.value.replace(',', '.')) || 0) })} type="number" min="0" step="0.01" aria-label="Valor" className="min-w-0 rounded border border-border bg-background px-2 py-1.5 text-[10px] text-foreground outline-none focus:border-primary" />
