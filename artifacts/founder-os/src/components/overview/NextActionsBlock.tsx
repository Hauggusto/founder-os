...
export function NextActionsBlock() {
  ...
  const save = () => { if (!text.trim()) return; addNextAction(text.trim(), project.trim() || undefined, priority, date); setText(''); setProject(''); setDate(new Date().toISOString().slice(0, 10)); setAdding(false); };
  ...
  return (
    <div className="rounded-2xl border border-card-border bg-card p-5">
      ...
      {adding && <div className="mb-4 grid gap-2 rounded-xl border border-primary/20 bg-primary/[.04] p-2 sm:grid-cols-[1fr_1fr_auto_auto_auto]"><input autoFocus value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') save(); if (e.key === 'Escape') setAdding(false); }} placeholder="Próxima ação" className="rounded-md border border-white/10 bg-background px-2 py-1.5 text-xs outline-none focus:border-primary" /><input value={project} onChange={(e) => setProject(e.target.value)} placeholder="Projeto / tag" className="rounded-md border border-white/10 bg-background px-2 py-1.5 text-xs outline-none focus:border-primary" /><input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="rounded-md border border-white/10 bg-background px-2 py-1.5 text-xs outline-none focus:border-primary" /><select value={priority} onChange={(e) => setPriority(e.target.value as typeof priority)} className="rounded-md border border-white/10 bg-background px-2 py-1.5 text-xs"><option value="important">Importante</option><option value="urgent">Urgente</option></select><button onClick={save} className="rounded-md bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground">Salvar</button></div>}
